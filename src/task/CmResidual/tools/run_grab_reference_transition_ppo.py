...
def main():
    ...
    horizon = 64 if args.window_length == 64 else 128
    ...
    common = [
        ...
        "headless=True", "num_envs=64", "pipeline=gpu", "sim_device=cuda:0", "rl_device=cuda:0",
        ...
        f"train.params.config.horizon_length={horizon}", "train.params.config.minibatch_size=2048",
    ...
    task = resolved["task"]
    if (task["env"]["numObservations"] != 68 or task["referenceStart"]["mode"] != "random_uniform" or
            ...
            bool(task["basePolicy"].get("useCmv2ActionEvaluator", False)) != args.cm_buffer):
    ...
    if not contract["passed"] or contract["actor_input_dim"] != 68 or contract["critic_input_dim"] != 68:
        raise RuntimeError(f"V1.14 model contract failed: {contract}")
    ...
    manifest = {"manifest_schema": "ref2dex.run.v1", "created_at": _now(), "task": "CmResidual",
                "mode": "v114_reference_transition_ppo", "run_id": args.run_id, "activity_id": args.activity_id,
                ...
                "cm_buffer": str(cm_buffer_dir) if args.cm_buffer else None,
                "budget": {"num_envs": 64, "window_length": args.window_length, "horizon_length": horizon,
                           "minibatch_size": 2048, "stages": [2, 10], "saturation_stop": 0.05},
    ...
    try:
        for label, end, expected in (("smoke", 2, 2), ("continuation", 10, 8)):
        ...
        manifest.update(run_status="COMPLETED", completed_at=_now(), checkpoint_validation=str(validation_path),
                        ...
                        exit_reason="Reached locked 2+8 epoch budget", conclusion="SUPPORTED", scientific_conclusion="INCONCLUSIVE"); _write_json(manifest_path, manifest)

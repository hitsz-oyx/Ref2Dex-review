"""Run and audit the CmResidual formal PPO training."""
...
HORIZON_LENGTH = 32
MINIBATCH_SIZE = 2048
EXPERIMENT_NAME = "rlgames_formal"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--smoke", action="store_true", help="1 env, zero-update initialization smoke")
    ...
    parser.add_argument(
        "--reference", type=Path,
        default=REPOSITORY_ROOT / "data/processed_data/cm_residual/reference_tracking_v2/s1_airplane_lift/reference.npz")
...
def _event_metrics(summary_dir: Path, expected_epochs: int) -> tuple[list[dict], list[str]]:
    ...
    if len(epoch_events) != expected_epochs:
        raise RuntimeError(f"Expected {expected_epochs} epochs, got {len(epoch_events)}")
    rows = []
...
def main() -> int:
    args = parse_args()
    run_id = args.run_id or f"cmresidual_ppo_formal_v17_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    max_epochs = 0 if args.smoke else args.max_epochs
    activity_id = args.activity_id or "ACT-CMRESIDUAL-V17-PPO-FORMAL"
    overrides = [
        "task=CmResidual",
        "train=CmResidualPPO",
        "headless=True",
        ...
        f"train.params.config.minibatch_size={32 if args.smoke else MINIBATCH_SIZE}",
        "pipeline=gpu",
        "sim_device=cuda:0",
        "rl_device=cuda:0",
    ...
    manifest = {
        ...
        "created_at": _now(),
        "mode": "formal_ppo_training" if not args.smoke else "formal_ppo_initialization_smoke",
        "task": "CmResidual",
        "run_id": run_id,
        ...
        "reference_allow_ineligible_for": "",
        ...
        "budget": {
            ...
            "minibatch_size": 32 if args.smoke else MINIBATCH_SIZE,
        },
    ...
    try:
        ...
        if args.smoke:
            manifest.update({"run_status": "COMPLETED", "completed_at": _now(),
                             "exit_reason": "Initialization smoke completed", "gate_passed": True,
                             "conclusion": "SUPPORTED", "scientific_conclusion": "INCONCLUSIVE"})
            _write_json(manifest_path, manifest)
            print(json.dumps({"run_id": run_id, "run_status": "COMPLETED", "gate_passed": True}, ensure_ascii=False), flush=True)
            return 0
        checkpoints = sorted((experiment_dir / "nn").glob("*.pth"), key=lambda path: path.stat().st_mtime_ns)
        if not checkpoints:
            raise RuntimeError(f"Formal run produced no checkpoint under {experiment_dir / 'nn'}")
        last_checkpoint = checkpoints[-1]
        best_checkpoint = experiment_dir / "nn/CmResidual.pth"
        rows, event_tags = _event_metrics(experiment_dir / "summaries", max_epochs)
        ...
        manifest.update({
            ...
            "exit_reason": "Reached formal PPO training budget",
            "gate_passed": passed,

...
- V1.21c：已按 [V1.21c 最终计划](plan/V1.21c.md) 实现 frozen Cmv2 的 P=1 native-action
  ranking core、state/physics/episode replay schema、duplicate-anchor/parity 与 episode-block
  bootstrap 统计，以及只读 replay metrics runner；合同测试通过。尚未启动正式 collector、PhysX
  ranking、PPO、P=3、异步 worker 或 Cmv2 更新，Cm 科学结论仍为 `INCONCLUSIVE`。实现与验证见
  [Activity](activities/ACT-20260920-151656-CMRESIDUAL-V121C-RANKING-CORE.md)。

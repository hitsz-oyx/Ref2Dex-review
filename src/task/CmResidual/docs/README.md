...
- `work_version`: `V1.21`
- V1.21：Phase 0 Cm-on engineering smoke、从零初始化的 10-epoch `cm_distill_coef=0` reference-PPO Phase A 和从其 checkpoint 出发的 Phase B 均已结束；V1.21b 已切换到 V1.20e DExplore 训练合同，Cm-off parity bootstrap 与四 rank x 64 env / 一实际 epoch DDP smoke 已通过（finite checkpoint）。下一步是 4 x 2048 容量门；Cmv2 geometry bridge、Cm-on、长 horizon、online planner、Cmv2 fine-tune 与抓取结论仍未授权。

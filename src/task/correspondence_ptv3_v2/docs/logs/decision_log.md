...
## 2026-09-03 — V2 采用固定手点槽位加有效 mask，并按数据集层级配额采样

- scope: task:correspondence_ptv3_v2
- anchor: `docs/指导/V2.md` / `V1.2.16`

**未指定点**

5 cm 内手点数量随帧变化，而现有 PTv3 接口要求固定 `num_hand_points=1538`；旧 sampler 只能让五个域等比例，无法表达 GRAB 与整体 HRDexDB 的 1:1 比例。

**实际选择**

保留 1538 个槽位并新增 `hand_valid_mask`，在特征、point-valid mask 和监督边中统一屏蔽无效槽位；扩展 `DomainBalancedSampler` 使用权重 `[0.5, 0.125, 0.125, 0.125, 0.125]`，新配置 batch size 40 对应 `[20,5,5,5,5]`。

**选择理由与影响**

该方案不破坏 checkpoint/model 的固定宽度接口，也不会复制或重排 MANO/机器人点索引；有效手点仍全部保留。加权 sampler 保证研究目标中的数据集比例，而不会被 frame 数量支配。无效槽位不再成为几何或监督信号。

**可逆性 / 是否需要用户确认**

完全可逆；旧配置省略 mask/weights 时分别回退为全手点有效和等域采样。用户已确认 5 cm 过滤、数据域和比例。

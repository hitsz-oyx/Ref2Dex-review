...
- work_version：`V1.13`；紧凑 endpoint shard schema、producer/reader、reference/compact 双后端和 DDP 接线已实现，Task 测试及五组单样本 parity 已通过。均衡 compact-cache pilot 已以 2000 records `COMPLETED`，覆盖五组 train stride 1/2/3 与全部 val stride 视图，`bad_count=0`且容量比 `0.577700 < 1.25`，full-build 机器门禁已通过。尚未构建 full cache，也未批准 benchmark、smoke 或正式训练。

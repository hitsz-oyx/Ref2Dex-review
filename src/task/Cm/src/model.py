...
class CmFlowHead(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.use_hand_flow_decoder = bool(use_hand_flow_decoder)
        self.hand_flow_target_scale = float(hand_flow_target_scale)
        self.allow_missing_hand_decoder_init = bool(allow_missing_hand_decoder_init)
        if min(
            self.geometry_input_scale,
            self.hand_flow_input_scale,
            self.object_flow_target_scale,
            self.hand_flow_target_scale,
        ) <= 0.0:
        ...
        # The hand-flow decoder is deliberately geometry-only at decode time.
        # It receives raw current hand xyz/normal, Cm, and Cm-relative anchor
        # geometry; z_hand, contact prior, and GT hand_flow are not exposed.
        hand_edge_input_dim = cm_dim + 12
        self.hand_edge_backbone = (
            nn.Sequential(
                nn.Linear(hand_edge_input_dim, cm_dim),
                nn.GELU(),
                nn.Linear(cm_dim, cm_dim // 2),
                nn.GELU(),
            )
            if self.use_hand_flow_decoder
            else None
        )
        self.hand_edge_flow_head = (
            nn.Linear(cm_dim // 2, 3)
            if self.use_hand_flow_decoder
            else None
        )
        if self.hand_edge_flow_head is not None:
            nn.init.zeros_(self.hand_edge_flow_head.weight)
            nn.init.zeros_(self.hand_edge_flow_head.bias)
    ...
    def forward(
        ...
        *,
        ...
        if self.hand_edge_backbone is not None and self.hand_edge_flow_head is not None:
            num_hand = hand_points.shape[1]
            hand_cm_edge = cm_tokens.unsqueeze(1).expand(-1, num_hand, -1, -1)
            hand_relative_position = (
                hand_points_internal.unsqueeze(2) - cm_anchor_pos_internal.unsqueeze(1)
            )
            hand_anchor_normal = cm_anchor_normal.unsqueeze(1).expand(
                -1, num_hand, -1, -1
            )
            raw_hand_geometry = torch.cat(
                [hand_points_internal, hand_normals], dim=-1
            )
            hand_edge_input = torch.cat(
                [
                    raw_hand_geometry.unsqueeze(2).expand(
                        -1, -1, self.num_dynamic_slots, -1
                    ),
                    hand_cm_edge,
                    hand_relative_position,
                    hand_anchor_normal,
            ...
            hand_edge_features = self.hand_edge_backbone(hand_edge_input)
            hand_slot_contribution_flow_scaled = self.hand_edge_flow_head(
                hand_edge_features
            )
            pred_hand_flow = hand_slot_contribution_flow_scaled.sum(dim=2)
            pred_hand_flow = pred_hand_flow / self.hand_flow_target_scale
            hand_contribution_norm = torch.linalg.vector_norm(
                hand_slot_contribution_flow_scaled, dim=-1
            )
            hand_norm_sum = hand_contribution_norm.sum(dim=2, keepdim=True)
            hand_uniform_usage = torch.full_like(
                hand_contribution_norm,
                1.0 / float(self.num_dynamic_slots),
            )
            hand_contribution_usage = torch.where(
                hand_norm_sum > 1e-8,
                hand_contribution_norm / hand_norm_sum.clamp_min(1e-8),
                hand_uniform_usage,
            )
            result.update(
                {
                    "pred_hand_flow": pred_hand_flow,
                    "hand_slot_contribution_flow": (
                        hand_slot_contribution_flow_scaled
                        / self.hand_flow_target_scale
                    ),
                    "hand_slot_contribution_usage": hand_contribution_usage,
                }
            )
        return result
...
class CmFlowModel(nn.Module):
        ...
        *,
        ...
        self.head = CmFlowHead(
            ...
            use_hand_flow_decoder=bool(
                getattr(meta, "use_hand_flow_decoder", False)
            ),
            hand_flow_target_scale=float(
                getattr(meta, "hand_flow_target_scale", 1.0)
            ),
            allow_missing_hand_decoder_init=bool(
                getattr(meta, "allow_missing_hand_decoder_init", False)
            ),
        )
    ...
    def load_state_dict(self, state_dict: dict[str, torch.Tensor], strict: bool = True):
        ...
        allowed_missing_prefixes = [self._DENSE_PREFIX]
        if self.head.allow_missing_hand_decoder_init:
            allowed_missing_prefixes.extend(
                ("head.hand_edge_backbone.", "head.hand_edge_flow_head.")
            )
        missing = [
            key

...
online in the current object-pose frame when ``obj_pose_world`` is available;
legacy caches fall back to the hand-root frame.
"""
...
def _evaluate_link_surface(vertices, faces, face_links, link_pose, face, bary):
    points, normals = evaluate_surface(vertices, faces, face, bary)
    pose = np.asarray(link_pose, dtype=np.float32)[np.asarray(face_links, dtype=np.int64)[np.asarray(face, dtype=np.int64)]]
    points = np.einsum("nij,nj->ni", pose[:, :3, :3], points) + pose[:, :3, 3]
    normals = np.einsum("nij,nj->ni", pose[:, :3, :3], normals)
...
def _resolve_episode_coordinate_frame(
    episodes: Sequence[tuple[str, Path]], *, requested: str | None = None
) -> str:
    """Require one pose convention across all selected HRDexDB episodes."""
    if not episodes:
        raise ValueError("Cannot resolve HRDexDB coordinate frame from an empty episode set")
    object_pose = [path for _, path in episodes if (path / "geometry" / "obj_pose_world.npy").is_file()]
    legacy = [path for _, path in episodes if not (path / "geometry" / "obj_pose_world.npy").is_file()]
    if object_pose and legacy:
        raise ValueError(
            "HRDexDB episode set mixes coordinate frames: "
    ...
    if requested is not None:
        ...
        if requested != detected:
            raise ValueError(
                f"Cm coordinate_frame={requested!r} disagrees with HRDexDB cache ({detected!r})"
            )
    return detected
...
class HrdexdbGeometryDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        min_stride: int = 1,
        max_stride: int = 10,
        stride_values: Sequence[int] | None = None,
        fixed_stride: int | None = None,
        active_only: bool = False,
        dataset_id: str = "hrdexdb",
        candidate_mask_name: str = "obj_candidate_mask_5cm.npy",
        ...
        self.candidate_mask_name = str(candidate_mask_name).strip()
        if not self.candidate_mask_name:
            raise ValueError("candidate_mask_name must be non-empty")
        self._epoch = mp.Value("q", 0, lock=True)
        self.episodes = [(str(name), Path(path)) for name, path in episodes]
        self.coordinate_frame = _resolve_episode_coordinate_frame(self.episodes, requested=coordinate_frame)
        ...
        for episode_index, (_, path) in enumerate(self.episodes):
            ...
            # Kept as a compatibility bitmap.  New caches mark the complete
            # object surface pool as valid instead of applying a 5cm query.
            candidate_path = geometry / self.candidate_mask_name
            if not candidate_path.is_file():
                raise FileNotFoundError(
                    f"HRDexDB episode {path} has no object-pool validity mask; rebuild the geometry layer with --num-obj-pool 4096"
                )
    ...
    def _load(self, episode_index: int) -> dict[str, np.ndarray]:
        ...
        if path not in self._cache:
            ...
            loaded = {
                ...
                "obj_pose": np.load(geometry / "obj_pose_world.npy", mmap_mode="r") if (geometry / "obj_pose_world.npy").exists() else None,
                "candidate": np.load(geometry / self.candidate_mask_name, mmap_mode="r"),
                "pose": np.load(geometry / "wrist_pose_world.npy", mmap_mode="r"),
                "hand_mesh": np.load(geometry / "hand_mesh_vertices_world.npy", mmap_mode="r") if (geometry / "hand_mesh_vertices_world.npy").exists() else None,
                "hand_mesh_local": np.load(geometry / "hand_mesh_vertices_local.npy", mmap_mode="r") if (geometry / "hand_mesh_vertices_local.npy").exists() else None,
                "hand_faces": np.load(geometry / "hand_mesh_faces.npy", mmap_mode="r") if (geometry / "hand_mesh_faces.npy").exists() else None,
                "hand_face_links": np.load(geometry / "hand_mesh_face_link_index.npy", mmap_mode="r") if (geometry / "hand_mesh_face_link_index.npy").exists() else None,
                "hand_link_pose": np.load(geometry / "hand_link_pose_world.npy", mmap_mode="r") if (geometry / "hand_link_pose_world.npy").exists() else None,
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor | str]:
        ...
        elif self.stride_values is not None:
            stride = int(rng.choice(self.stride_values))
        else:
            stride = int(rng.integers(self.min_stride, self.max_stride + 1))
        ...
        if self.coordinate_frame == "object_pose_t":
            if data["obj_pose"] is None:
                raise ValueError(f"{episode_name}: object_pose_t requested but obj_pose_world.npy is missing")
        ...
        if self.active_only and not bool(valid.any()):
            raise ValueError(f"HRDexDB episode has no valid object-surface point: {episode_name} frame={current}")
        ...
        if data["hand_mesh"] is not None and data["hand_mesh"].shape[1] > 0 and data["hand_faces"] is not None:
        ...
        elif data["hand_mesh_local"] is not None and data["hand_mesh_local"].shape[0] > 0 and data["hand_faces"] is not None and data["hand_face_links"] is not None and data["hand_link_pose"] is not None:
            hand_face, hand_bary = sample_surface_spec(data["hand_mesh_local"], data["hand_faces"], self.num_hand_points, seed ^ 0xA11CE)
            hand_world, hand_normals_world = _evaluate_link_surface(data["hand_mesh_local"], data["hand_faces"], data["hand_face_links"], data["hand_link_pose"][current], hand_face, hand_bary)
            hand_future_world, _ = _evaluate_link_surface(data["hand_mesh_local"], data["hand_faces"], data["hand_face_links"], data["hand_link_pose"][future], hand_face, hand_bary)
            hand_now = _world_to_hand(hand_world, pose)
            hand_future = _world_to_hand(hand_future_world, pose)
            hand_normals = _normal_world_to_hand(hand_normals_world, pose)
        else:
...
def make_dataloaders(data_cfg: Any, seed: int, *, meta_cfg: Any, distributed=None):
    ...
    requested_coordinate_frame = str(getattr(meta_cfg, "coordinate_frame", "hand_root_t"))
    hrdexdb_specs = {
        split: _manifest_specs(hroot, manifest, split, source_prefix)
        for split in ("train", "val", "test")
    }
    coordinate_frame = _resolve_episode_coordinate_frame(
        [item for specs in hrdexdb_specs.values() for item in specs],
        requested=requested_coordinate_frame,
    )
    ...
    hcommon["candidate_mask_name"] = str(getattr(data_cfg, "hrdexdb_candidate_mask_name", "obj_candidate_mask_5cm.npy"))
    hcommon["coordinate_frame"] = coordinate_frame
    is_inspire_f1 = bool(source_prefix and source_prefix.rstrip("/").lower().startswith("inspire_f1"))
    configured_hrdexdb_train_strides = getattr(data_cfg, "hrdexdb_train_strides", None)
    hrdexdb_train_strides = None
    if configured_hrdexdb_train_strides is not None:
        hrdexdb_train_strides = tuple(sorted({int(value) for value in configured_hrdexdb_train_strides}))
        if not hrdexdb_train_strides or any(value <= 0 for value in hrdexdb_train_strides):
            raise ValueError("data.hrdexdb_train_strides must contain positive integers")
        hcommon["stride_values"] = hrdexdb_train_strides
        hcommon["max_stride"] = max(int(hcommon["max_stride"]), max(hrdexdb_train_strides))
    elif is_inspire_f1:
        # Preserve the historical fixed-stride=2 behavior for configs that do
        # not opt into an explicit Inspire-F1 stride set.
        hcommon["fixed_stride"] = 2

    if hrdexdb_only:
        htrain = HrdexdbGeometryDataset(
            hrdexdb_specs["train"], **hcommon
        )
        ...
        val_strides = tuple(int(value) for value in getattr(data_cfg, "hrdexdb_val_strides", getattr(data_cfg, "val_strides", (1, 5, 10))))
        test_strides = tuple(int(value) for value in getattr(data_cfg, "hrdexdb_test_strides", getattr(data_cfg, "test_strides", val_strides)))
        val_loaders = {}
        test_loaders = {}
        for stride in val_strides:
            eval_common = dict(hcommon)
            eval_common["fixed_stride"] = stride
        ...
        metadata = {
            ...
            "dataset_split": {
                "hrdexdb_train_episodes": len(hrdexdb_specs["train"]),
                "hrdexdb_val_episodes": len(hrdexdb_specs["val"]),
                "hrdexdb_test_episodes": len(hrdexdb_specs["test"]),
            },
    ...
    htrain = HrdexdbGeometryDataset(hrdexdb_specs["train"], **hcommon)
    ...
    hrdexdb_val_strides = tuple(int(value) for value in getattr(data_cfg, "hrdexdb_val_strides", val_strides))
    hrdexdb_test_strides = tuple(int(value) for value in getattr(data_cfg, "hrdexdb_test_strides", hrdexdb_val_strides))
    if not val_strides or not test_strides or not hrdexdb_val_strides or not hrdexdb_test_strides:
    ...
    for stride in hrdexdb_val_strides:
        eval_common = dict(hcommon)
        eval_common["fixed_stride"] = stride
        hval = HrdexdbGeometryDataset(hrdexdb_specs["val"], **eval_common)
    ...
    for stride in hrdexdb_test_strides:
        eval_common = dict(hcommon)
        eval_common["fixed_stride"] = stride
        htest = HrdexdbGeometryDataset(hrdexdb_specs["test"], **eval_common)
        test_loaders[f"test/stride_{stride}/{hrdexdb_bucket_name}/"] = eval_loader(htest)
    metadata = {"schema_name": "ref2dex_cm_hrdexdb_finetune_v1", "coordinate_frame": coordinate_frame,
                "num_obj_pool": int(getattr(meta_cfg, "num_obj_pool", 4096)),
                "num_obj_points": int(meta_cfg.num_obj_points),
                "num_hand_points": int(meta_cfg.num_hand_points),
                ...
                "dataset_split": {"base_train_sequences": len(base_train), "hrdexdb_train_episodes": len(hrdexdb_specs["train"])},
                "val_strides": list(val_strides), "test_strides": list(test_strides),
                "hrdexdb_train_strides": list(hrdexdb_train_strides) if hrdexdb_train_strides is not None else [2] if is_inspire_f1 else None,
                "hrdexdb_val_strides": list(hrdexdb_val_strides), "hrdexdb_test_strides": list(hrdexdb_test_strides)}

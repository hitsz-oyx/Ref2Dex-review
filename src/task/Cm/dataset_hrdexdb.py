"""HRDexDB-to-Cm adapter and balanced GRAB/ARCTIC/HRDexDB mixture.
...
from src.task.Cm.dataset_object_v2 import _MmapSequenceDataset, _resolve_splits, _sequence_dirs
...
def make_dataloaders(data_cfg: Any, seed: int, *, meta_cfg: Any, distributed=None):
    ...
    their existing ``dataset_id`` into GRAB and ARCTIC buckets; no additional
    per-embodiment HRDexDB balancing is applied.
    """
    ...
    train_dataset = BalancedCmMixture({"grab": train_sources["grab"], "arctic": train_sources["arctic"], "hrdexdb": htrain}, probs, seed=seed)
    train_kwargs = make_dataloader_kwargs(data_cfg, seed, drop_last=False)
    ...
    for stride in val_strides:
        ...
        hval = HrdexdbGeometryDataset(
            _manifest_specs(hroot, manifest, "val"), fixed_stride=stride, **hcommon
        )
        val_loaders[f"val/stride_{stride}/hrdexdb/"] = eval_loader(hval)
    for stride in test_strides:
        ...
        htest = HrdexdbGeometryDataset(
            _manifest_specs(hroot, manifest, "test"), fixed_stride=stride, **hcommon
        )
        test_loaders[f"test/stride_{stride}/hrdexdb/"] = eval_loader(htest)
    metadata = {"schema_name": "ref2dex_cm_hrdexdb_finetune_v1", "coordinate_frame": "hand_root_t",
                "num_obj_pool": 512, "num_obj_points": 512, "num_hand_points": 1538,
                "source_probabilities": probs, "hrdexdb_manifest": str(manifest),
                "dataset_split": {"base_train_sequences": len(base_train), "hrdexdb_train_episodes": len(_manifest_specs(hroot, manifest, "train"))},

...
rollout。由于 OICM 的 5 cm hard interaction contract 不定义语义 null-Cm，产物同时保存：
...
CUDA_VISIBLE_DEVICES=3 /home2/wyy/miniconda3/envs/graspenv/bin/python \
  ...
  --config src/task/CmDecoderv2/configs/active/dexplore_rl_v1_2_5.yaml \
  --checkpoint outputs/cmdecoderv2/cm_decoder_v2_dexplore_rl_v1_2_5_20260908_101402/checkpoints/best.pt \
  --device cuda:0 \
...
`run_manifest.json`。`effect.npz` 中的 `gt_obj_flow_*`、`gt_effect_rms_mm` 和
`pred_gt_effect_epe_mm` 均为 display-only 字段。

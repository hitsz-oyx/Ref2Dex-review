# 修改影响等级参考

等级描述的是影响范围，而不是行数。三行坐标系修改也可能是 L2；不提交的大型诊断产物仍可能是 L0。

## L0 — 常规修改

文档、测试、格式化、只读诊断和不改变运行变量的配置元数据。无需用户审批，但要记录修改和验证。改变训练变量的配置、数据选择或运行合同不属于 L0；实验配置应新建变体，不要直接改写基线。

## L1 — 任务内实现

任务内代码修改，保持输入/输出 schema、GT 含义、坐标系、数据划分、指标和 checkpoint 解释不变。增加定向测试，并保持原入口兼容。

## L2 — 科研语义或公共合同

修改数据字段、采样、GT、坐标系、单位、训练/验证/测试边界、指标、缓存 schema、checkpoint 兼容性或公共接口声明。说明旧语义与拟议语义，等待用户批准后再编辑。

## L3 — 共享治理或基础设施

修改共享 base runtime、仓库治理、依赖、数据迁移、破坏性操作、外部服务或长时任务。实现前给出范围、风险、回滚和验证计划。

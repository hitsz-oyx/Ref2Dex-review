...
4. `docs/current_versions.yaml`（存在时）、根级 `docs/logs/activity_log.md`、`architecture_log.md`、`repo_memory.md`；
5. 当前 Task 的 `docs/README.md`、`docs/logs/activity_log.md`、指导和对应数字后缀的已定稿 plan；
6. `git status --short`、当前 diff，以及涉及运行目录的进程状态。

按任务追加：

- 设计或历史兼容：`decision_log.md`、`activity_log.md`、历史 `modification_log.md`；
...
`python .agents/skills/research-change-control/scripts/audit_diff.py --log <activity_log> --staged --check-links`。
回复中的路径链接与前后中文或标点之间留半角空格；运行终态不得留下缺失目标。

每次实质操作都要确认 `modification_version` 和操作类别；参数并行实验只新增活动和独立

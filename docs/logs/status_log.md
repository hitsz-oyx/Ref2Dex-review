...
- 当前进行中: Cm 保留 GRAB C=32 geometry-only/no-time 与双卡 GRAB C=64 容量对照；CmDecoder Inspire F1 全量 object-disjoint 3 Hz point-flow decoder 仍在训练。通用 Component/Artifact/Contract/ExecutionContext/PipelineSpec 原型已加入 `src/base`，当前三条主线已有只读 manifest，尚未迁移执行逻辑。
...
- 证据与相关文档: 全量 pytest `289 passed, 3 skipped`；[仓库修改记录](modification_log.md)、[通用组件架构](architecture_log.md)、[correspondence 状态](../../src/task/correspondence_ptv3_v2/docs/logs/status_log.md)、[Cm 状态](../../src/task/Cm/docs/logs/status_log.md)。

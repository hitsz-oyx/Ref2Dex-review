...
class BaseTask():
    ...
    def __init__(self, cfg, enable_camera_sensors=False):
        ...
        # Disable graphics device when running headless without cameras
        ...
        self.contact_reset = torch.zeros(
            (self.num_envs, 5), device=self.device, dtype=torch.float)
        self.extras = {}
    ...
    def step(self, actions):
        ...
        # Fetch simulation results on CPU (GPU results are fetched in render)
        if self.device == 'cpu':

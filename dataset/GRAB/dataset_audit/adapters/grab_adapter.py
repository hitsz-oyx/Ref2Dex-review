...
def build_manifest(repo_root: Path, limit: int | None = None) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    data_roots = candidate_data_roots(repo_root, ["GRAB_DATASET_PATH", "GRAB_PATH"], ["data/grab", "grab_data", "GRAB_data"])
    existing_roots = [path for path in data_roots if path.exists()]
    for root in existing_roots:
        seqs = sorted(root.rglob("*.npz"))[:10]
        for seq_path in seqs:
            rows.append(
                {
                    "dataset_name": DATASET_NAME,
                    "item": _rel(root, seq_path),
                    "subject": seq_path.parent.name,
                    "sequence": seq_path.stem,
                    "object_name": "",
                    "data_available": True,
                    "cache_path": "",
                    "source_path": _rel(repo_root, seq_path),
                    "status": "grab_npz_available",
                    "message": "GRAB sequence NPZ exists; official SMPL-X/MANO/object assets are required to instantiate mesh cache.",
                }
            )
    if not rows:
        rows.append(
            {
                ...
                "source_path": "",
                "status": "data_missing",
                "message": "No GRAB_DATASET_PATH or local data/grab directory found. Tooling will not download data or models.",
            }
        )
    ...
    summary = {
        ...
        "contact_source": "GRAB provides binary body-object contact maps: each object vertex is labeled with no-contact or body/hand part contact.",
    }
    return rows, summary
...
def prepare_sequence_cache(repo_root: Path, *, item: str, manifest_path: Path, out_cache: Path | None) -> dict[str, Any]:
    ...
    raise SystemExit(
        "GRAB sequence data must be processed with official SMPL-X/MANO/object loaders inside Docker to instantiate body/object meshes. "
...
def enrich_cache(repo_root: Path, cache_path: Path, cache: dict[str, Any]) -> dict[str, Any]:
    ...
    out["contact_available"] = True
    out.setdefault(
        "contact_source",
        "GRAB official body-object binary contact map: object vertices labeled by no-contact or body/hand part.",
    )
    return out


def object_mesh_paths(repo_root: Path, limit: int | None = None) -> list[Path]:
    roots = candidate_data_roots(repo_root, ["GRAB_OBJECT_ROOT", "GRAB_DATASET_PATH"], ["data/object_meshes", "tools/object_meshes", "data"])
    paths: list[Path] = []
    for root in roots:
        if root.exists():
            paths.extend(sorted(root.rglob("*.obj")))
            paths.extend(sorted(root.rglob("*.ply")))
...
def inspect_contact_info(repo_root: Path, max_hits: int = 80) -> dict[str, Any]:
    return {
        "dataset_name": DATASET_NAME,
        "has_official_contact_info": True,
        "contact_available": "official_binary_contact_map",
        "contact_source": [
            "README: GRAB contains binary contact maps between body and objects.",
            "For each motion frame, each object vertex is associated with 0 for no contact or a body/hand-part id.",
            "This is a proximity/contact label, not a signed-distance penetration or triangle collision test.",
        ],
        "is_penetration_or_collision_test": False,
        "notes": [
            "GRAB contact maps can be compared against geometric negative-distance statistics after mesh cache generation.",
            "Do not interpret the contact map as penetration depth.",
        ],
...
def _scan_keywords(repo_root: Path, max_hits: int) -> list[dict[str, str]]:
    terms = ("contact", "proximity", "touch", "penetration", "collision")
    roots = [repo_root / "README.md", repo_root / "grab", repo_root / "tools", repo_root / "examples"]

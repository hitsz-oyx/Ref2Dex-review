...
- 已定稿方案：`src/task/<Task>/docs/plan/V<n>.md` 及适用的字母后缀、点数字微补充
...
指导补充不自动要求新建 plan，也不与 plan 一一对应。已有 `a`、`b`、`c` 指导后才协商定稿的计划，仍使用 `plan/V<n>.md`。状态不是 `FINAL` 的 plan 可在原文件原地修订。计划一旦 `FINAL`，后续任何改变可执行范围、保护项、风险、验证或回滚合同的修改都不得覆盖原文件，通常必须建立 `plan/V<n><letter>.md`。由 `指导/V<n><letter>.md` 触发时使用同一字母；没有对应指导时使用该 plan 谱系中下一个未使用的小写字母。仅澄清意图、未改变实施合同的指导补充不产生 plan；后缀 plan 仍须经用户确认。

若一个字母后缀 plan 已实际执行但小任务未达到预期，后续只是在**同一分支、同一研究问题和同一保护边界**内做有界纠偏或探索重试，且用户明确选择不新建分支，可建立点数字微补充 `plan/V<n><letter>.<k>.md`，例如 `V1.21e.1.md`。`k` 从 `1` 开始、不得有前导零，并在同一父 plan 下按整数连续递增；不得用 `.1` 跳过尚不存在的父字母 plan。微补充不推进 `work_version`，不用于指导文件命名，也不把独立研究问题、扩大保护边界或无界追加工作伪装成小修订；发生这些情况时仍使用下一个字母 plan，并按通常规则判断是否需要新分支。

plan 的读取与冲突顺序为：无后缀基线、字母 `a` 至 `z`；每个适用字母 plan 后紧接其 `.1`、`.2`……微补充，数字按整数而非文件名字典序排序。较后节点只覆盖明确冲突部分，其他约束累计生效。点数字 plan 仍须记录复用分支的理由、改变与保持项、风险、验证、回滚和用户批准；它不降低 L2/L3 审批或正式 run 门禁。

开始 change、run 或 governance 时，先读取完整指导谱系，再读取基线最终 plan、被后续指导实际触发的已确认字母补充及其适用点数字微补充。指导与计划无法同时满足时，停止实施并按上述冲突流程处理。

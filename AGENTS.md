...
- `modification_version`：已确认的工作/变更边界；根版本用于治理或跨 Task 合同，Task 版本用于局部工作。
- `git_commit`：具体实现身份；每个运行和重要活动都记录可复现的提交哈希。
- `run_id`：一次训练、评估、benchmark 或数据处理运行的身份。

`modification_version` 不是每次命令、测试、轮询或 checkpoint 的编号；运行使用独立 run_id，具体实现使用 git_commit。

`Vn.m.k` 可以作为同一工作边界下的可选细分编号，通常对应一个 branch、Activity 和 PR；它不因命令、测试或轮询自动递增。
...
`ai/<task>/<effective-guidance>-<description>` 分支，完成实现、验证和交接后再合并。实验参数变化不创建分支。
...
- Task `docs/README.md` 是当前状态页，记录 modification_version、研究问题、已接受架构、invariant、blocker、证据
  入口和重要 entrypoint；它不是历史流水账。
- `指导/V<n>.md` 及其字母补充指导由用户维护研究目标、假设、成功标准和禁止事项；仅在用户确认同文件冲突的修订
  措辞后，AI 才可修改相应指导。
- 只有新的 modification_version 边界或 L2/L3 方案需要最终 `plan/V<n>.md`；L0/L1 修复不为每次任务创建 plan。
...
- `activities/` 的独立 Activity 记录具有长期追溯价值的完成工作单元；`activities/README.md` 只做索引。
  重要实现、正式指导修订、重要诊断事实、重要操作、版本或治理切换应写 Activity；单行修复、命令、逐 step
...
Activity 至少包含 timestamp、activity_id 或细分版本、modification_version、base_commit、分支、scope、
...
1. inspect：读取最小上下文，确认 Task、modification_version、保护边界和当前 Git 状态。
2. change：读取全部适用指导并审查冲突；新研究方案或 L2/L3 先形成并确认 plan；在任务分支实施最小差异并补定向测试。
3. run：读取全部适用指导并审查冲突，固定代码提交、modification_version、假设、变量、指标、预算和停止条件；生成独立 run_id、配置快照和 run_manifest，
   结束后记录 activity 与实验卡。
